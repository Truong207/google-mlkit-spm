...
create_xcframework() {
    ...
    # Ký .xcframework sau khi tạo
    FRAMEWORK_PATH="$OUTPUT_PATH/$TARGET_NAME.xcframework"
    CODESIGN_IDENTITY="3F701EC80A7205978607A4FB8B4D530A75AC0CBC" # <-- chỉnh lại identity này

    echo "Signing .xcframework for target: ${GREEN}$TARGET_NAME${NC}.."
    find "$FRAMEWORK_PATH" -type d -name "*.framework" | while read -r FRAMEWORK; do
        echo "  Signing: $FRAMEWORK"
        codesign --force --timestamp --sign "$CODESIGN_IDENTITY" "$FRAMEWORK"
    done
